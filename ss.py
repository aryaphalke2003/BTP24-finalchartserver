...
class DetectionObject:
    def __init__(self, xlt, ylt, xrb, yrb, role, text,mapped_tick=None,axis=None,val=None):
        self.xlt = xlt
        self.ylt = ylt
        self.xrb = xrb
        self.yrb = yrb
...
def reverse_func(x_min, y_min, x_max, y_max):
...
def unnormalize(x_ticks,img_path):
...
def run_script(filename,image_path):
    ...
    latest_exp_folder = get_latest_exp_folder(exp_folder)
    folderpath = os.path.join(latest_exp_folder, 'labels')
    barplots_lines = read_single_txt_in_folder(folderpath)
    os.chdir('..')
    ...
    for line in label_lines:
        ...
        if line[0] == '7':
            ...
            output_filename = f'temp_image_{c}.jpg'
            c+=1 
    ...
    barps=[]
    for line in barplots_lines:
        line = line.rstrip()
        line = line.split(" ")
        barps.append(line[1]+' '+line[2]+' '+line[3]+' '+line[4])
        
    
    if len(barps)>0:
        parsed_entries = [list(map(float, entry.split())) for entry in barps]
        barps = sorted(parsed_entries, key=lambda x: x[0])
    ...
    x_ticklabels = sorted(x_ticklabels,key=lambda x: x[0])
    x_ticks = sorted(x_ticks,key=lambda x: x[0])
    y_ticklabels = sorted(y_ticklabels,key=lambda x: x[1])
    y_ticks = sorted(y_ticks,key=lambda x: x[1])
    
    yy_ticks=y_ticks
    xx_ticks=x_ticks
    yy_ticklabels=y_ticklabels
    xx_ticklabels=x_ticklabels
    
    print("xx_ticks",xx_ticks)
    unfxx  = []
    for i in xx_ticks:
        a,b,c,d = i
        unfxx.append(a,b,c,d)
        
    print("unfuncxx",unfxx)
    ...
    for tick in y_ticks:
        ...
        # nearest_ticklabel = y_ticklabels[c]
        cx+=1
    ...
    for tick in x_ticks:
        ...
        # nearest_ticklabel = x_ticklabels[c]
        cx+=1
        aa,bb,cc,dd = nearest_ticklabel
        img_height, img_width, _ = img.shape
        a,b,c,d = int(aa * img_width), int(bb * img_height), int(cc * img_width), int(dd * img_height)
    ...
    val_per_pix = abs(float(bobj.text)-float(aobj.text))/abs(bobj.ylt-aobj.ylt)
    
    print("val_per_pix",val_per_pix)
    
    xx_ticks = sorted(xx_ticks,key=lambda x: x[0])
    ...
    for i in barps:
        print("in barps ",i)
        a, b, c, d = i
        val = int(d * val_per_pix)  # Convert to integer as specified
        print("val",val)
        if cou < len(xx_ticks):
            value_mapping[xx_ticks[cou]]= val  # Update the value part of the tuple
        cou += 1
    
    for obj in label_objects:
        if obj.axis == 'x':
            obj.val = value_mapping[obj.mapped_tick]
