...
class CBAMMM(nn.Module):
    # Standard convolution with args(ch_in, ch_out, kernel, stride, padding, groups, dilation, activation)
    #

    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        super(CBAMMM, self).__init__()
        self.depthwise = nn.Conv2d(c1, c1, k, s, autopad(k, p, d), groups=c1, dilation=d, bias=False)
        self.pointwise = nn.Conv2d(c1, c2, k) 
        self.bn = nn.BatchNorm2d(c2)
...
class C3CBAMM(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(CBAMM(c_, c_,shortcut) for _ in range(n)))      
...
class C3CBAMMM(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(CBAMMM(c_, c_,shortcut) for _ in range(n))) 
...
class DWConv(Conv):
    ...
    def __init__(self, c1, c2, k=1, s=1, d=1, act=True):  # ch_in, ch_out, kernel, stride, dilation, activation
        super().__init__(c1, c2, k, s, g=math.gcd(c1, c2), d=2, act=act)
